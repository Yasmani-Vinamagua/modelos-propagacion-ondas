import customtkinter as ctk
import numpy as np
import estilos
from error import VentanaError

app = ctk.CTk()

fuentes = estilos.cargar_fuentes() #Se llama a la funcion q tiene el diccionario de fuentes

class Walfisch:

    def __init__(self, frame_inputs, frame_OptionMenu):
        self.frame_OptionMenu = frame_OptionMenu
        self.frame_inputs = frame_inputs

        self.operacion = 0.00

        self.valores = {} # Creamos una lista para almacenar los datos del input

        self.var_walfisch = {
            "Pérdida (dB)": (0, 0, "Pérdida total (PL) en dB"),
            "Pérdida en espacio libre L₀ (dB)": (0, 0, "Pérdida en espacio libre (L₀)"),
            "Pérdida por techo a calle Lᵣₜₛ (dB)": (0, 0, "Pérdida de techo a calle (Lᵣₜₛ)"),
            "Pérdida por múltiples difracciones Lₘₛd (dB)": (0, 0, "Pérdida por múltiples difracciones (Lₘₛd)")
        }

        self.list_menu = list(self.var_walfisch.keys())  # Opciones de cálculo modelo walfisch

        self.entradas_walfisch = []  # Lista para guardar los imputs del modelo walfisch


    def menu(self):

        # Menú de opciones para modelo okomura
        self.optionmenu = ctk.StringVar(value=self.list_menu[0])
        self.optionwalfisch = ctk.CTkOptionMenu(self.frame_OptionMenu,
                                             values=self.list_menu,
                                             variable=self.optionmenu,
                                             width=250, height=30,
                                             command= self.inputs) #cada vez que el usuario cambia la selección,
        # se llama automáticamente a la función inputs pasando el valor seleccionado como argumento.
        self.optionwalfisch.grid(padx=32, pady=5)

        self.inputs(self.list_menu[0]) #Llama a la funcion inputs por primera vez para mostrar los inputs de la primera opción al cargar la interfaz por defecto.


    #Entradas del modelo walfisch
    def inputs(self, seleccion):

        self.seleccion_modelo = seleccion #Guarda el valor seleccionado del menú

        # Limpiar todos los inputs
        for widget in self.frame_inputs.winfo_children():
            widget.destroy()

        self.entradas_walfisch.clear()

        fila = 0

        # Genera los labels e Imputs del modelo
        for self.variable in self.list_menu:
            if self.seleccion_modelo != self.variable: #Si el valor del menu seleccionado es diferente a la lista de opciones imprime

                self.label_walfisch = ctk.CTkLabel(self.frame_inputs, text=self.variable, text_color="black", font=fuentes["text_resalt"])
                self.label_walfisch.grid(row=fila, column=0,**estilos.pad_labels)

                self.in_walfisch = ctk.CTkEntry(self.frame_inputs, width=estilos.ancho_inputs,
                                               placeholder_text=self.var_walfisch[self.variable][2], #Imprimer de la lista de var_walfisch[key][item]
                                          font=fuentes["text"])
                self.in_walfisch.grid(row=fila, column=1,**estilos.pad_inputs)

                self.entradas_walfisch.append(self.in_walfisch)  # Almacenamos los imputs en una lista

            fila += 1

    #Calculo de variables
    def calculo(self):

        #Refrescamos los valores
        self.valores.clear()
        self.operacion = 0

        variables = [v for v in self.list_menu if v != self.seleccion_modelo] #Generamos una lista con todas las variables menos la seleccionada

        for nombre_variable, entrada in zip(variables, self.entradas_walfisch): #Asosiamos cada nombre de variable con su valor ingresado

            try:
                self.valor = float(entrada.get()) #Covertimos cada valor ingresado en un float

                min_val, max_val, _ = self.var_walfisch[nombre_variable]

                if min_val !=0 or max_val!=0: #Evitar validar aquellas variables q no tienen rango como a(hm) o pl
                    if not (min_val <= self.valor <= max_val):
                        VentanaError(app) #Valor fuera del rango lanza pantalla error

                self.valores[nombre_variable] = self.valor #Creamos un diccionario con cada nombre variable y float ingresado

            except ValueError:
                self.valores[nombre_variable] = None #Si se ingresa otro valor q no sea numerico se llena con un None

        if None in self.valores.values():
            return VentanaError(app) #Si existe un valor none en valores{} lanza la ventana de error

        l0 = self.valores.get("Pérdida en espacio libre L₀ (dB)")
        Lrts = self.valores.get("Pérdida por techo a calle Lᵣₜₛ (dB)")
        Lmsd = self.valores.get("Pérdida por múltiples difracciones Lₘₛd (dB)")
        pl = self.valores.get("Perdida (dB)")


        # Evalúa cada opción de cálculo del modelo Walfisch
        if self.seleccion_modelo == "Pérdida (dB)":
            self.operacion = L0 + Lrts + Lmsd

        elif self.seleccion_modelo == "Pérdida en espacio libre L₀ (dB)":
            self.operacion = PL - Lrts - Lmsd

        elif self.seleccion_modelo == "Pérdida por techo a calle Lᵣₜₛ (dB)":
            self.operacion = PL - L0 - Lmsd

        elif self.seleccion_modelo == "Pérdida por múltiples difracciones Lₘₛd (dB)":
            self.operacion = PL - L0 - Lrts